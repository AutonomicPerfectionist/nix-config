...
let
  cfg = config.goblin-frpc.services.rocketchat;
...
in
{
  options.goblin-frpc.services.rocketchat = {
    enable = lib.mkEnableOption "Enable the rocketchatVTT service";
    hostname = mkOption {
      type = types.str;
    };
    volumePath = mkOption {
  ...
  config = lib.mkIf cfg.enable {
    ...
    #   image = "felddy/rocketchatvtt:sha-1a166b7";
