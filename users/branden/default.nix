{
  config,
  lib,
  pkgs,
  flake-inputs,
  ...
}:
let
  cfg = config.userconfig.branden;
in
{

  imports = [
    # pending https://discourse.nixos.org/t/services-kanata-attribute-lib-missing/51476
    ../../modules/nix-dev.nix
    flake-inputs.home-manager.nixosModules.default
  ];

  options.userconfig.branden = {
    enable = lib.mkEnableOption "user branden";
    graphical = lib.mkEnableOption "graphical desktop environment for user branden";
    hostname = lib.mkOption {
      defaultText = "hostname of the current system";
      type = lib.types.str;
    };
  };

  config = lib.mkIf cfg.enable {
    environment.etc.certfile = {
      source = "/etc/ssl/certs/ca-bundle.crt";
      target = "ssl/cert.pem";
    };
    users.users.branden = {
      isNormalUser = true;
      description = "Branden Butler";
      extraGroups = [
        "networkmanager"
        "wheel"
        "video"
        "render"
        "docker"
      ];
      shell = pkgs.zsh;

      openssh.authorizedKeys.keys = [
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIB6ErD21kz5qWhGQLJSKO9qlKnH44LU0yS4rAd8+OQkC branden@big-nix"
        "ssh-rsa  branden@trash-can"
        "ssh-rsa  branden@ryzen-desktop"
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIIJ1hx3H03bpsmZSLH9EpM42jDYkU/U6OvT/+QOSO0lM branden@cursed-steel"
        "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJFNJ+gAVHqQAzERuIhedFubY5pGDccx5qnrUWxyZyVg branden@battle-bucket"
      ];

      initialHashedPassword = "$y$j9T$5mvVE9Wt4ayuvT91YtZbS/$8zGpGOPuu1t4GfzM9USIngyiv32I26z.C5laHrqjTH2";
      # hashedPasswordFile = config.age.secrets.branden-passwd.path;
    };

    # age.secrets."branden-passwd" = {
    # 		file = ../../secrets/branden-passwd.age;
    # 		owner = "branden";
    # 	};
    # TODO these programs.* options are pretty scattered,
    # should reorganize
    programs.zsh.enable = true;
    programs.nix-ld.enable = true; # Needed for foreign dylib programs
    environment.pathsToLink = [ "/share/zsh" ];
    home-manager = {
      users.branden = ./home/home.nix;
      extraSpecialArgs = {
        inherit flake-inputs;
        userConfiguration = cfg;
      };
      sharedModules = lib.optionals cfg.graphical [
        flake-inputs.niri.homeModules.niri
        flake-inputs.noctalia.homeModules.default
      ];
    };
  };
}
